...
export class NormasComponent implements OnInit, OnDestroy {
  ...
  constructor(
    protected normasService: NormasService,
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.normasService
      ...
      .subscribe(
        (res: HttpResponse<INormas[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInNormas(): void {
    this.eventSubscriber = this.eventManager.subscribe('normasListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: INormas[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
